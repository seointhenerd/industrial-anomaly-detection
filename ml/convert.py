...
samples = np.array([
    [26.5, 41.9],  # typical normal
    [27.0, 42.5],  # normal
    [26.2, 41.3],  # normal
    [30.2, 85.0],  # borderline (real anomaly from dataset)
    [45.0, 95.0],  # extreme anomaly
], dtype=np.float32)
...
for i, (raw, norm) in enumerate(zip(samples, samples_norm)):
    interpreter.set_tensor(input_details[0]["index"], norm.reshape(1, 2))
    interpreter.invoke()
